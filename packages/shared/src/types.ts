...
export type BookKind = 'txt' | 'epub' | 'pdf' | 'mobi';
...
export interface ReadingProgress {
  locator: Locator;
  lineProgress: number; // 0-1, speech/reading line progress
  updatedAt: string;
}
...
export interface UpdateBookPayload {
  title?: string;
  author?: string;
  cover?: string; // base64 data URL to set, or '' to remove
}
...
export interface TxtRenditionDescriptor {
  kind: 'txt';
  segments: TxtSegment[];
}
...
export type RenditionDescriptor = EpubRenditionDescriptor | PdfRenditionDescriptor | TxtRenditionDescriptor | MobiRenditionDescriptor;
...
export type PageView = 'scroll' | 'single';
...
export const DEFAULT_READER_SETTINGS: ReaderSettings = {
  fontSize: 16,
  fontFamily: 'Georgia, "Noto Serif SC", "Source Han Serif", serif',
  lineHeight: 1.5,
  paragraphSpacing: 1,
  indent: 2,
  pageView: 'single',
