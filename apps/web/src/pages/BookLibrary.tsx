...
import { useBooks, useDeleteBook, useUpdateBook, useUploadBook } from '@/services/queries';
import { BookOpen, CirclePlus, Loader, Moon, Sun } from 'lucide-react';
...
export const BookLibrary = () => {
  ...
  return (
    <main className={cn('h-full w-full pt-8 pb-30 px-8 md:px-20 xl:px-[20%]', 'flex flex-col gap-2', 'text-sm text-center')}>
      ...
      {/* No Books */}
      {books.length === 0 && uploads.length === 0 && !isLoading && (
      ...
      <div className="py-2 flex flex-wrap gap-2 justify-center md:justify-start">
        ...
        {books.map((doc) => (
          <BookItem
            ...
            onUpdate={(data) => updateBook.mutate({ id: doc.id, data })}
            onDelete={() => deleteBook.mutate(doc.id)}
          />
        ))}
      </div>
    </main>
